...
export default function PersonDetailPage() {
  ...
  const createLedgerEntry = async () => {
    setLedgerError("");
    const rate = Number(ledgerForm.goldRate) || getDefaultGoldRate();
    if (!ledgerForm.grossWeight || !ledgerForm.purity || !rate) {
      setLedgerError("Gross weight, purity and gold rate are required.");
      return;
    }
    ...
    try {
      const res = await fetch(`/api/persons/${params.id}/gold-ledger`, {
        ...
        body: JSON.stringify({
          ...
          wastageWeight: Number(ledgerForm.wastageWeight),
          goldRate: rate,
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              {["Date", "Particulars", "Category", "Type", "Issued", "Receipt", "Dr/Cr", "Cash Balance", "Gold Balance", "Notes", "Bill"].map((h) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl border border-border bg-card p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <h4 className="text-lg font-semibold">Add Gold Ledger Entry</h4>
              <button onClick={() => setShowAddLedger(false)} className="rounded-md px-2 py-1 text-sm font-medium text-muted-foreground hover:bg-muted">Cancel</button>
            </div>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={() => setShowAddLedger(false)} className="rounded-lg border border-border px-3 py-2 text-sm">Cancel</button>
