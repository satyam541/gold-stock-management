...
import {
  ...
  CARAT_OPTIONS, TRANSACTION_TYPES,
} from "@/lib/utils";
...
export default function GoldPage() {
  ...
  const handleSubmit = async () => {
    setError("");
    if (!form.personId) { setError("Please select or type a person name."); return; }
    if (!form.weight || parseFloat(form.weight) <= 0) { setError("Please enter a valid weight."); return; }
    setSubmitting(true);
    try {
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex gap-2">
          <select value={filterCarat} onChange={(e) => setFilterCarat(e.target.value)} className="h-9 rounded-lg border border-border bg-card px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30">
            ...
            {CARAT_OPTIONS.map((c) => <option key={c.value} value={c.value}>{c.value}</option>)}
          </select>
